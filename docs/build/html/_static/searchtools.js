 ...
 */
const _escapeRegExp = (string) => string.replace(
...
const _displayItem = (item, searchTerms, highlightTerms) => {
  ...
  if (descr) {
    ...
    if (SPHINX_HIGHLIGHT_ENABLED) // set in sphinx_highlight.js
      highlightTerms.forEach(
          (term) => _highlightText(listItem, term, "highlighted"));
...
const _finishSearch = (resultCount) => {
  ...
  if (!resultCount)
    ...
    Search.status.innerText = _(`Search finished, found ${
        resultCount} page(s) matching the search query.`);
};
...
if (typeof splitQuery === "undefined") {
  var splitQuery = (query) =>
      query.split(/[^\p{Letter}\p{Number}_\p{Emoji_Presentation}]+/gu)
...
const Search = {
  ...
  query : (query) => {
    ...
    if (SPHINX_HIGHLIGHT_ENABLED) { // set in sphinx_highlight.js
      localStorage.setItem(
          "sphinx_highlight_terms", [...highlightTerms ].join(" "))
    }
    ...
    results = results.reverse().reduce((acc, result) => {
      let resultStr = result.slice(0, 4)
                          .concat([ result[5] ])
                          .map(v => String(v))
                          .join(',');
  ...
  performObjectSearch : (object, objectTerms) => {
    ...
    const objectSearchCallback = (prefix, match) => {
      const name = match[4] const fullname =
          (prefix ? prefix + "." : "") + name;
      const fullnameLower = fullname.toLowerCase();
      if (fullnameLower.indexOf(object) < 0)
        return;
    ...
    Object.keys(objects).forEach(
        (prefix) => objects[prefix].forEach(
            (array) => objectSearchCallback(prefix, array)));
    return results;
  },
  ...
  performTermsSearch : (searchTerms, excludedTerms) => {
    ...
    searchTerms.forEach((word) => {
      ...
      if (arr.every((record) => record.files === undefined))
        return;
    ...
    for (const [file, wordList] of fileMap) {
      ...
      if (wordList.length !== searchTerms.size &&
          wordList.length !== filteredTermCount)
        continue;

      // ensure that none of the excluded terms is in the search result
      if ([...excludedTerms ].some(
              (term) => terms[term] === file || titleTerms[term] === file ||
