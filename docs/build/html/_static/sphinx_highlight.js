...
const _highlight = (node, addItems, text, className) => {
  if (node.nodeType === Node.TEXT_NODE) {
    ...
    if (pos >= 0 && !parent.classList.contains(className) &&
...
const _highlightText = (thisNode, text, className) => {
  ...
  addItems.forEach(
      (obj) => obj.parent.insertAdjacentElement("beforebegin", obj.target));
};
...
const SphinxHighlight = {
  ...
  highlightSearchWords : () => {
    ...
    if (searchBox === null)
      return;
    searchBox.appendChild(document.createRange().createContextualFragment(
        '<p class="highlight-link">' +
        '<a href="javascript:SphinxHighlight.hideSearchWords()">' +
        _("Hide Search Matches") + "</a></p>"));
  },
   ...
   */
  hideSearchWords : () => {
    document.querySelectorAll("#searchbox .highlight-link")
